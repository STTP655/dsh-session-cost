...
.sc-mask {
	...
	/* 遮罩只作为定位层和"点外部关闭"的命中区，不压暗也不模糊；玻璃感交给卡片本身。 */
	background: transparent;
	animation: sc-mask-in var(--ds-transition-duration, 180ms) cubic-bezier(0.23, 1, 0.32, 1) both;
}
.sc-mask[data-phase="closing"] {
	animation: sc-mask-out ${EXIT_MS}ms cubic-bezier(0.23, 1, 0.32, 1) both;
}
.sc-card {
	position: relative;
	display: flex;
	...
	/* 玻璃卡片：先把主题层色作为兜底，再用 color-mix 降到 78% 不透明；
	   模糊沿用 DSH 菜单的配方。不支持 color-mix 的浏览器会停在上面那行，仍是不透明卡片。 */
	...
	overflow: hidden;
	animation: sc-card-in 220ms cubic-bezier(0.23, 1, 0.32, 1) both;
}
.sc-mask[data-phase="closing"] .sc-card {
	animation: sc-card-out ${EXIT_MS}ms cubic-bezier(0.23, 1, 0.32, 1) both;
}
