...
.sc-mask {
	...
	animation: sc-mask-in var(--ds-transition-duration, 180ms) var(--ds-ease-in-out, ease) both;
}
.sc-mask[data-phase="closing"] {
	animation: sc-mask-out ${EXIT_MS}ms ease-in both;
}
.sc-card {
	position: relative;
	display: flex;
	...
	overflow: hidden;
	animation: sc-card-in 220ms cubic-bezier(.22, 1, .36, 1) both;
}
.sc-mask[data-phase="closing"] .sc-card {
	animation: sc-card-out ${EXIT_MS}ms ease-in both;
}
@keyframes sc-mask-in { from { opacity: 0 } to { opacity: 1 } }
@keyframes sc-mask-out { from { opacity: 1 } to { opacity: 0 } }
...
@media (prefers-reduced-motion: reduce) {
	.sc-mask, .sc-card, .sc-mask[data-phase="closing"] .sc-card { animation-duration: 1ms }
}
.sc-head {
	display: flex;
...
`;
		...
		const internals = {
			...
			hourRows, groupHours, initialLedger, accumulate, totalsOf, loadLedger, saveLedger,
		...
		/** 一个分区：标题 + 若干行 + 小计。 */
		function detailSection(t, title, rows, cost, total) {
			return (0, react_jsx_runtime.jsxs)("div", {
				children: [
		...
		function CostDetail(props) {
			...
			if (grouped.recent.length > 0) {
				sections.push(detailSection(t, t("recentTitle"), rows, grouped.recentCost, total));
			}
