window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 高峰时段：北京时间周一至周五（不含法定节假日）9:00-12:00、14:00-18:00；周末与节假日全天按空闲计。 */
		function isPeakBeijing(now) {
			const bj = new Date(now.getTime() + 8 * 3600e3);
			const t = bj.getUTCHours() + bj.getUTCMinutes() / 60;
			return bj.getUTCDay() !== 0 && bj.getUTCDay() !== 6 && ((t >= 9 && t < 12) || (t >= 14 && t < 18));
		}
		...
		/** 测试钩子。 */
		const internals = { FLAT_PRICES, TIERED_PRICES, NEW_PRICING_AT, DEFAULT_MODEL, PEAK_COLOR, OFF_COLOR, computeCost, isPeakBeijing, billedTotal };
